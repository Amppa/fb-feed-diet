...
- **Zero backward-compatibility overhead during rapid dev**: as established in [DEVELOPMENT.md](../DEVELOPMENT.md), the project maintains zero defensive code for legacy schemas or abandoned settings keys. When configs evolve, merge `FB_DIET_DEFAULTS.SETTINGS` with current keys or reset to defaults. Never introduce speculative migration wrappers.
